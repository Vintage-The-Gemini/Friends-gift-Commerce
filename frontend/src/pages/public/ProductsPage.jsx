...
const OptimizedProductCard = React.memo(({ 
  ...
  const handleAddToEventClick = useCallback((e) => {
    e.stopPropagation();
  ...
  useEffect(() => {
    if (product.images?.[0]?.url && index < 8) { // Preload first 8 images
      const img = new Image();
...
const ProductsPage = () => {
  ...
  const fetchProducts = useCallback(async (resetPage = false, loadMore = false) => {
    try {
      ...
      const response = await productService.getAllProducts(apiFilters, options);
                  ...
                  {categories.map((category) => (
                    <label key={category._id} className="flex items-center cursor-pointer hover:bg-gray-50 p-2 rounded">
                      <input
                        ...
                        onChange={() => setFilters(prev => ({ ...prev, category: category._id }))}
                        className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                      />
